...
public sealed class MessageStoreMappingExtensionsTests
{
    ...
    [Theory]
    [InlineData(nameof(OutboxMessageEntity.Status))]
    [InlineData(nameof(OutboxMessageEntity.NextRetryAtUtc))]
    public void MapOutboxMessage_TheClaimALeaseHolderHolds_IsAConcurrencyToken(string propertyName) =>
        Assert.True(OutboxProperty(propertyName).IsConcurrencyToken);

    [Theory]
    [InlineData(nameof(OutboxMessageEntity.Payload))]
    [InlineData(nameof(OutboxMessageEntity.Attempts))]
    public void MapOutboxMessage_WhatTheClaimDoesNotGuard_IsNotAConcurrencyToken(string propertyName) =>
        Assert.False(OutboxProperty(propertyName).IsConcurrencyToken);
