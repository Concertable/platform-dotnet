...
**Progress — PR1 (package, `Chore/TechDebt`) landed the producer half:** `ConnectionString` is now validated in the `ServiceBusClient` singleton factory (fault on first resolution, not at registration), `ConnectionString` became `string?` and `ServiceName` became `null!` (this also closes the root `TECH_DEBT.md` `= "" → null!` item), and the eager `ServiceName` fail-fast is kept. Unit-covered in `AddAzureServiceBusTransportTests`. **Remaining — PR2 (delivery-gated on the republish + `platform-sync` pin bump, since all 7 hosts consume `Concertable.Messaging.AzureServiceBus` as a pinned `PackageReference`):** drop the `?? (IsIntegration() ? null! : throw …)` asb connection-string guard to a plain `opts.ConnectionString = GetConnectionString("asb")` in B2B.Web, B2B.Seed.Simulator, Customer.Web, Search.Workers, Payment.Web, Payment.Workers, and Auth — then delete this entry.
