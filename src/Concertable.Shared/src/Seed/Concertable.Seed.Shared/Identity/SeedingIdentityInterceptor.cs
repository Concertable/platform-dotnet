...
public sealed class SeedingIdentityInterceptor : DbCommandInterceptor
{
    ...
    private void Rewrite(DbCommand command, CommandEventData e)
    {
        if (!scope.IsActive || e.Context is null ||
            ...
            !SeedingIdentityRewriter.RequiresIdentityInsert(e.Context.Database.ProviderName))
            return;

        var identityTables = tableCache.GetOrAdd(
            e.Context.GetType(),
            _ => SeedingIdentityRewriter.BuildTableMap(e.Context.Model));

        if (SeedingIdentityRewriter.Rewrite(command.CommandText, identityTables) is { } rewritten)
