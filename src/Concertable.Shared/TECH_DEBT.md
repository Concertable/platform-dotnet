...
The interceptor is fixed, the three workarounds are deleted, and its SQL rewrite is unit-covered. What
follows is the remaining tiering, cheapest first:

1. **A dev-seeder smoke test per service in the integration tier.** Every `ApiFixture` already has
...
2. **An architecture test for seeder parity.** Every `ITestSeeder` has an `IDevSeeder` twin for the same
...
3. **Collapse the dev/test seeder pair onto one implementation** differing only by interface, so a fix
...
**Resolves when:** each service's integration suite runs its `IDevSeeder` chain against a migrated database,
and an architecture test asserts dev/test seeder parity with no hand-written `IDENTITY_INSERT` in either.
Item 3 is the structural follow-up and may be tracked separately.
